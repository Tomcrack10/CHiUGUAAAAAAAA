...
client.on("messageReactionAdd", async (reaction, user) => {
	...
	Schema.findOne({ Message: reaction.message.id }, async(err, data) => {
		...
		user.send(`**${reaction.message.guild.name}: Acabas de obtener el rol ${dmRole}!**`).catch(err => {
...
client.on("messageReactionRemove", async (reaction, user) => {
	...
	Schema.findOne({ Message: reaction.message.id }, async(err, data) => {
		...
		user.send(`**${reaction.message.guild.name}: Acabas de perder el rol ${dmRole}!**`).catch(err => {
