...
module.exports = {
    ...
    const query = new MessageEmbed()
	    ...
	    suggestedEmbed.embeds[0].fields[2].value = `Aceptado - ${replyQuery}`;
	    suggestedEmbed.embeds[0].color = "GREEN";



            suggestedEmbed.edit({ embeds: [suggestedEmbed.embeds[0]] });
             message.reply(":white_check_mark: Se ha aceptado la sugerencia.\n" + suggestedEmbed.embeds[0].fields[2].value)

            const user = await client.users.cache.find((u) => u.tag === data.author.name)

            const embed = new MessageEmbed()
                .setDescription(`Tu [sugerencia](https://discord.com/channels/${message.guild.id}/${channel}/${messageID}) ha sido aceptada.`)
                .setColor('GREEN')
